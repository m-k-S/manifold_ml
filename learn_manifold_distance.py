...
npts = 15
pts = [[] for _ in range(npts)]
mean = np.zeros(2)
Q = np.diag([2, 2])

I = np.diag([1, 1])
for i in range(npts):
    pts[i].append(np.random.multivariate_normal(mean, Q))
    pts[i].append(np.random.multivariate_normal(mean, Q))
...
for pair in pts:
    ...
    hx = hyp_mfd(x)
    hy = hyp_mfd(y)
    true.append(hyp_mfd_dist(hx, hy, None))
    for m in range(1, 21):
        test = learn_distance(x, y, I, integrand_hyp, 100, m)
        p_s.append(test)

scipy.io.savemat('hyperboloid_path_approx.mat', mdict = {'arr': p_s, 'true': true})
